...
//----note to self. core of application contains function----
//ToolMain Class
ToolMain::ToolMain()
{
...
void ToolMain::onActionSave()
{
	...
	StopEditingObjects();
	// from this point we can not undo or redo actions that where taken from before the save
	// but we are now free to add new actions to the stack
...
void ToolMain::UpdateCamSettings(float moveSpeed, float camRotationSpeed, float mouseSensitivity)
{
	m_d3dRenderer.SetCameraValues(moveSpeed, camRotationSpeed, mouseSensitivity);
}
...
{
	if (HasFocus()) {
		//----note to self recive button presses----
		switch (msg->message)
		{
